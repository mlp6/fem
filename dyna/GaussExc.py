...
def main():
    ...
    parser.add_option("--sigma1",dest="sigma1",type="float",help="S.D. in dimension 1 [default = %default]",default=1.0)
    parser.add_option("--sigma2",dest="sigma2",type="float",help="S.D. in dimension 2 [default = %default]",default=1.0)
    parser.add_option("--sigma3",dest="sigma3",type="float",help="S.D. in dimension 3 [default = %default]",default=1.0)
    ...
    loadfilename = "gauss_exc_sigma1_%.3f_sigma2_%.3f_sigma3_%.3f_center_%.3f_%.3f_%.3f_amp_%.3f_amp_cut_%.3f_%s.dyn" % (opts.sigma1,opts.sigma2,opts.sigma3,opts.center[0],opts.center[1],opts.center[2],opts.amp,opts.amp_cut,opts.sym)
    LOADFILE = open(loadfilename,'w')
    LOADFILE.write("$ Generated using %s with the following options:\n" % sys.argv[0])
    LOADFILE.write("$ %s\n" % opts)
    ...
    for i in NODEFILE:
        ...
        if i[0] != "$" and i[0] != "*":
            ...
            exp1 = math.pow(float(fields[1])-opts.center[0],2)
            exp2 = math.pow(float(fields[2])-opts.center[1],2)
            exp3 = math.pow(float(fields[3])-opts.center[2],2)
            ...
            if nodeGaussAmp > opts.amp*opts.amp_cut:
                ...
                if opts.sym is "qsym":
                    if float(fields[1]) < opts.search_tol and float(fields[2]) < opts.search_tol:
                        nodeGaussAmp = nodeGaussAmp/4
                        sym_node_count += 1
                    elif float(fields[1]) < opts.search_tol or float(fields[2]) < opts.search_tol:
                        nodeGaussAmp = nodeGaussAmp/2
                        sym_node_count += 1

                # check for half symmetry force reduction (if needed)
                if opts.sym is "hsym":
                    if float(fields[1]) < opts.search_tol:
                        nodeGaussAmp = nodeGaussAmp/2
                        sym_node_count += 1
