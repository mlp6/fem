#!/usr/bin/perl
# zdispOnAxis.pl - extract the node z-disp histories "on axis" into individual
# ASCII files to be read into a post-processor.  ls-prepost2 is used to extract
# the data from d3plot files since node history data is in different formats
# when run using ls-dyna vs. mpp-dyna
#
# INPUTS: None (but run-time variables are defined below)
# OUTPUTS: zdispOnAxis directory is created with individual ASCII files named by
...
my $x = 0; # x & y coordinates that define "on axis"
my $y = 0; 
my $nodeFileName = 'ph41nodes.dyn';
my $outputDir = 'zdispOnAxis';
...
# create a hash of the output nodes (not sorted yet); save the z-coordinate for sorting
foreach our $nodeID (@nodeIDs) {
    our %outputNodes; 
    if (abs($nodeCoords{$nodeID}[0] - $x) < $SearchTol && abs($nodeCoords{$nodeID}[1] - $y) < $SearchTol) {
        $outputNodes{$nodeID} = $nodeCoords{$nodeID}[2];
    }
}
...
# sort output nodes by increasing z-coordinate
