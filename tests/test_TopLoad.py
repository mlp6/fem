...
def test_extract_top_plane_nodes():
    ...
    planeNodeIDs = extract_top_plane_nodes(nodefile=nodefile, top_face=[0, 0, 0, 0, 0, 1])
...
def test_writeNodeLoads_disp(tmpdir):
    ...
    writeNodeLoads(loadfile=f.strpath, planeNodeIDs=[[1, 2 ,3], [4, 5, 6]], loadtype='disp', direction=2,
...
def test_writeNodeLoads_force(tmpdir):
    from TopLoad import writeNodeLoads
    f = tmpdir.join("topload.dyn")
    writeNodeLoads(loadfile=f.strpath, planeNodeIDs=[[1, 2 ,3], [4, 5, 6]], loadtype='force', direction=2,
                   amplitude=-1.0, lcid=1)
    lines = f.readlines()
