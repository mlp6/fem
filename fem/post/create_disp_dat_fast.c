...
char *inFileName = "nodout", *outFileName = "disp.dat";
int debug = 0;
int legacyNodes = 0;
...
int i;
	...
	create_disp_dat(inFileName, outFileName, debug, legacyNodes);
 ...
 */
				...
				nodeCnt = 0;
				if (fwrite(dispVals[0], sizeof(float), numNodes, outptr) != numNodes) {
					fprintf(stderr, "failed to write x data\n");
					exit(EXIT_FAILURE);
					}
				if (fwrite(dispVals[1], sizeof(float), numNodes, outptr) != numNodes) {
					fprintf(stderr, "failed to write y data\n");
					exit(EXIT_FAILURE);
					}
				if (fwrite(dispVals[2], sizeof(float), numNodes, outptr) != numNodes) {
