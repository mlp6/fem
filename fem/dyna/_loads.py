...
class DynaMeshLoadsMixin:
    def add_load_curve(self, load_curve_id, load_type, load_timing_args):
        ...
        load_timing_str = ''
        for a, o in t:
            load_timing_str += f"{format_dyna_number(a, 20):>20}{format_dyna_number(o, 20):>20}\n"
    ...
    def make_loads_temps(self, load_curve_id, arf_push_duration, cv, arf_intensity_file, normalization_isppa=None, normalization_intensity_file=None):

        if normalization_isppa is not None and normalization_intensity_file is not None:
            raise ValueError("Cannot provide both normalization_isppa and normalization_intensity_file.")
        
        if normalization_isppa is None and normalization_intensity_file is None:
            raise ValueError("Must provide either normalization_isppa or normalization_intensity_file.")
