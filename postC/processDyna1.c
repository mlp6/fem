...
float *dispVals, header[HEADER_SIZE];
double temp;
FILE *outptr;
int c, currStep, dispValsIndex;
 ...
 */
	...
	dispVals = (float *)malloc(sizeof(float) * totalNumValsPerStep);
	memset(dispVals, 0, numNodes * sizeof(float));
 ...
 */
				...
				c = 0;
				if (fwrite(dispVals, sizeof(float), totalNumValsPerStep, outptr) != totalNumValsPerStep) {
					fprintf(stderr, "failed to write header\n");
					exit(EXIT_FAILURE);
					}
				memset(dispVals, 0, numNodes * sizeof(float));
 ...
 */
				...
				*(dispVals + dispValsIndex++) = (float )temp;
...
/* write current values */
	...
	else if (fwrite(dispVals, sizeof(float), totalNumValsPerStep, outptr) != totalNumValsPerStep) {
		fprintf(stderr, "failed to write header\n");
		exit(EXIT_FAILURE);
		}
